...
performers = [ "IBM-MIT-Harvard-Stanford", "MESS-UCBerkeley", "OPICS (OSU, UU, NYU)" ]
agent_tasks = ["agents efficient action path lure", "agents efficient action time control", "agents object preference"]
...
def find_pair_doc(documentscopy, scene_part_num, ground_truth):
    ...
    for new_doc in documentscopy:
        if scene_part_num == new_doc["scene_part_num"] and new_ground_truth == new_doc["score"]["ground_truth"]:
            return new_doc
...
def process_pairing_scoring(performer: str, agent_task: str):
    ...
    documents = list(collection.find({"eval": EVAL_NAME, "performer": performer, "category_type": agent_task}))

    for doc in documents:
        pair_doc = find_pair_doc(documents, doc["scene_part_num"], doc["score"]["ground_truth"])
        pair_doc_plaus = 0
        if pair_doc is not None:
            pair_doc_plaus = scene_plausibility(pair_doc["score"]["classification"], pair_doc["score"]["confidence"])
        else:
            print("not found: " + doc["name"])
        doc["score"]["adjusted_confidence"] = scene_plausibility(doc["score"]["classification"], doc["score"]["confidence"])
        ...
        result = collection.update_one({"_id": ObjectId(doc["_id"])}, {"$set": doc}, True)

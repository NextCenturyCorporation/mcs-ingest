...
The scorecard has unit tests in ```tests/test_scorecard.py```.

Some of the unit tests are based on output scene history files in test_data.  
Those history files are generated by code in ```tests/test_data_generator/```.  
and the output compared  with the values in
```tests/test_data/scorecard_ground_truth.txt```.  The scene history files 
...
To use the relook generator:

```
% PYTHONPATH='.' python test_data_generator/scorecard_generate_relook.py [MCS_unity] [scene_file]
```
